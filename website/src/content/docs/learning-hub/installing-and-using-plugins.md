...
*(v1.0.76+)* The `/plugins` command gives you fine-grained control over which components are active. You can enable or disable individual plugins, instructions, agents, LSP servers, and hooks without uninstalling them:

```
/plugins
```

Opening `/plugins` shows a list of all loaded components. Toggle any item on or off — disabled components are skipped at runtime but remain installed, so you can re-enable them at any time. This is useful when you want to temporarily suppress a hook or agent for a specific session without permanently removing the plugin.
